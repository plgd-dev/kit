...
func TestClaims(t *testing.T) {
	...
	assert.Equal(t, 2019, time.Unix(c.ExpiresAt, 0).Year())
	assert.Empty(t, c.Id)
	assert.Equal(t, time.Unix(0, 0), time.Unix(c.IssuedAt, 0))
	assert.Equal(t, c.Issuer, "http://identity-server:3001")
	assert.Equal(t, 2019, time.Unix(c.NotBefore, 0).Year())
...
func TestParser(t *testing.T) {
	...
	assert.Equal(t, "test.client.id", c["client_id"])
	assert.Equal(t, "user@example.com", c["email"])
	assert.Contains(t, c["scope"], "test.scope")

	assert.Equal(t, "local", c["idp"])
	assert.Contains(t, c["amr"], "pwd")
	assert.Contains(t, c["aud"], "http://identity-server:3001/resources")
	assert.Contains(t, c["aud"], "test.resource")
	assert.Equal(t, "http://identity-server:3001", c["iss"])
	assert.Equal(t, "1b87effa-34e2-4a44-82c6-6e0ab80209ff", c["sub"])

	assert.Equal(t, 2019, time.Unix(int64(c["nbf"].(float64)), 0).Year())
	assert.Equal(t, 2019, time.Unix(int64(c["exp"].(float64)), 0).Year())
