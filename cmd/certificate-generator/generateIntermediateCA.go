...
func newCert(opts Options) (*x509.Certificate, error) {
	notBefore := time.Now()
	notAfter := notBefore.Add(opts.Certificate.ValidFor)
	...
	if opts.Certificate.MaxPathLen >= 0 {
		if opts.Certificate.MaxPathLen == 0 {
			template.MaxPathLenZero = true
		} else {
			template.MaxPathLen = opts.Certificate.MaxPathLen
		}
	}
	return &template, nil
}
...
func generateIntermediateCA(opts Options, privateKey *ecdsa.PrivateKey, signerCA []*x509.Certificate, signerCAKey *ecdsa.PrivateKey) ([]byte, error) {
